...
{
    public enum OverlayType { Default, Pistol, Rifle }
    ...
    public class Weapon : FPSItem
    {
        ...
        [Header("Recoil (no ScriptableObject)")]
        [SerializeField] private RecoilProfile recoilProfile;          // <-- CHANGED: top-level type
        [SerializeField] private RecoilPatternAsset recoilPatternAsset; // optional deterministic spray
        ...
        private static readonly int OverlayTypeHash = Animator.StringToHash("OverlayType");
        ...
        {
            foreach (var g in scopeGroups) g.Initialize(_fpsAnimator);
            _scopeIndex = 0;
            if (scopeGroups.Count == 0) return;
            UpdateAimPoint();
        ...
        public override void OnEquip(GameObject parent)
        {
            ...
            _controllerAnimator.SetFloat(OverlayTypeHash, (float)overlayType);
            ...
            // Init procedural recoil from per-weapon profile
            if (_recoilAnimation != null)
            {
                _recoilAnimation.useDeterministicPattern = (recoilPatternAsset != null);
                _recoilAnimation.pattern = recoilPatternAsset;
                _recoilAnimation.Init(recoilProfile, fireRate, _fireMode);
            }
        ...
        {
            _controllerAnimator.SetFloat(OverlayTypeHash, 0);
        ...
        {
            _controllerAnimator.SetFloat(OverlayTypeHash, (int)overlayType);
        ...
        public override bool OnAimPressed()
        {
            ...
            if (_recoilAnimation != null) _recoilAnimation.isAiming = true;
            return true;
        }
        ...
        public override bool OnAimReleased()
        {
            ...
            UpdateTargetFOV(false);
            if (_recoilAnimation != null) _recoilAnimation.isAiming = false;
            return true;
        }
        ...
        {
            // rate limit
            if (Time.unscaledTime - _lastRecoilTime < 60f / fireRate) return false;
        ...
        {
            if (!FPSAnimationAsset.IsValid(reloadClip)) return false;

            _playablesController.PlayAnimation(reloadClip, 0f);

            if (_weaponAnimator != null) { _weaponAnimator.Rebind(); _weaponAnimator.Play("Reload", 0); }
            if (_fpsCameraController != null) _fpsCameraController.PlayCameraAnimation(cameraReloadAnimation);
        ...
        {
            if (!FPSAnimationAsset.IsValid(grenadeClip)) return false;

            _playablesController.PlayAnimation(grenadeClip, 0f);
            if (_fpsCameraController != null) _fpsCameraController.PlayCameraAnimation(cameraGrenadeAnimation);
        ...
        private void OnFire()
        {
            if (_weaponAnimator != null) _weaponAnimator.Play("Fire", 0, 0f);
            ...
            {
                _bursts--;
                if (_bursts == 0) { OnFireReleased(); return; }
            }
        ...
        public override void OnCycleScope()
        {
            ...
            _scopeIndex++;
            if (_scopeIndex > scopeGroups.Count - 1) _scopeIndex = 0;
        ...
        {
            if (_fireMode == FireMode.Semi && supportsBurst) { _fireMode = FireMode.Burst; _bursts = burstLength; return; }
            if (_fireMode != FireMode.Auto && supportsAuto)   { _fireMode = FireMode.Auto; return; }
        ...
        {
            CycleFireMode();
            if (_recoilAnimation != null) _recoilAnimation.fireMode = _fireMode;
        }

        public override void OnAttachmentChanged(int attachmentTypeIndex)
        {
            if (attachmentTypeIndex == 1) { barrelAttachments.CycleAttachments(_fpsAnimator); return; }
            if (attachmentTypeIndex == 2) { gripAttachments.CycleAttachments(_fpsAnimator);   return; }
