...
using UnityEngine.UIElements; // <-- INVENTORY TOGGLE (UITK)
...
namespace Demo.Scripts.Runtime.Character
{
    ...
    public class FPSController : MonoBehaviour
    {
        ...
        [Header("Inventory / UI")]
        [SerializeField] private UIDocument inventoryUIDocument;          // Assign your inventory UI UIDocument (e.g., UI_Root)
        [SerializeField] private string inventoryRootName = "InventoryRoot"; // Root element name inside your UXML
        [SerializeField] private bool pauseGameWhenInventoryOpen = false; // Optional pause

        private VisualElement _inventoryRoot;
        private bool _isInventoryOpen;
        // --- INVENTORY TOGGLE (fields end) ---
        ...
        // UPDATED to treat inventory as an "active action"
        private bool HasActiveAction()
        {
            return _actionState != FPSActionState.None || _isInventoryOpen;
        }
        ...
        private void Start()
        {
            ...
            // --- INVENTORY TOGGLE (init UI root) ---
            if (inventoryUIDocument != null)
            {
                _inventoryRoot = inventoryUIDocument.rootVisualElement.Q<VisualElement>(inventoryRootName);
                inventoryUIDocument.rootVisualElement.RegisterCallback<PointerDownEvent>(e =>
                    Debug.Log($"[UITK] click -> {(e.target as VisualElement)?.name}")
                );

                if (_inventoryRoot == null)
                {
                    Debug.LogWarning(
                        $"FPSController: '{inventoryRootName}' not found in {inventoryUIDocument.name}. " +
                        "Toggle will only unlock the cursor.");
                }
                else
                {
                    _inventoryRoot.style.display = DisplayStyle.None; // start hidden
                }
            }
            // --- INVENTORY TOGGLE (init end) ---
        }
        ...
        {
            if (newIndex == _activeWeaponIndex || newIndex > _instantiatedWeapons.Count - 1) return;
        ...
        {
            // --- INVENTORY TOGGLE: freeze look while inventory is open ---
            if (_isInventoryOpen)
            {
                _userInput.SetValue(FPSANames.MouseDeltaInput, Vector4.zero);
        ...
        private void Update()
        {
            // Respect optional pause when inventory open
            Time.timeScale = (_isInventoryOpen && pauseGameWhenInventoryOpen) ? 0f : settings.timeScale;
            UpdateLookInput();
        }

        // --- INVENTORY TOGGLE (logic) ---
        private void ToggleInventory(bool? force = null)
        {
            bool open = force ?? !_isInventoryOpen;
            if (open == _isInventoryOpen) return;
            ...
            // Cancel combat-ish states when opening
            if (open)
            {
                OnFireReleased();
                DisableAim();
            ...
            // Show/hide UITK inventory root if assigned
            if (_inventoryRoot != null)
            {
                _inventoryRoot.style.display = open ? DisplayStyle.Flex : DisplayStyle.None;
                if (open) _inventoryRoot.Focus();
            }
        }
        // --- INVENTORY TOGGLE (logic end) ---
        ...
        {
            if (IsSprinting() || _isInventoryOpen) return;
        ...
        {
            if (_isInventoryOpen) { _lookDeltaInput = Vector2.zero; return; }
        ...
        // Add an action named "Inventory" in your Input Actions (Button) and bind it to Tab.
        ...
        // --- INVENTORY TOGGLE (input binding end) ---
#endif
    }
}
